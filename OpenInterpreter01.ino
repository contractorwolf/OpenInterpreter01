...
class ButtonChecker
{
  public:
      ...
      void loop()
      {
          ...
          thisTickState = digitalRead(buttonPin) == LOW; // Assuming the button is active LOW
      }
...
void webSocketEvent(WStype_t type, uint8_t *payload, size_t length)
{
    ...
    case WStype_TEXT:
        Serial.printf("[WSc] get text: %s\n", payload);
        {
            ...
            {
                M5.dis.drawpix(0, CRGB(0, 128, 0));// green
            ...
            else if (isAudio && str.find("\"end\"") != std::string::npos)
            {
                ...
                M5.dis.drawpix(0, CRGB(0, 128, 150));// cyan
...
void setup() {
    ...
    M5.dis.drawpix(0, CRGB(255, 0, 50)); // Set the display color. red
...
void loop()
{
    ...
    if (WiFi.status() == WL_CONNECTED && hasSetupWebsocket)
    {
        ...
        {
            M5.dis.drawpix(0, CRGB(255, 255, 0));// yellow
    ...
    if(WiFi.status() != WL_CONNECTED) {
      if (millis() - last_try_wifi > WIFI_INTERVAL) {
          M5.dis.drawpix(0, CRGB(255, 0, 50)); // Set the display color. red
          last_try_wifi = millis();            // seems to help with stability, if you are doing other things in the loop this may not be needed
          tryReconnectWiFi();
      }
    }
}
